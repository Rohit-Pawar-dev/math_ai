...
const ProfileManagement = () => {
  ...
  const fetchUsers = async (pageNo = 1, searchText = '') => {
    try {
      ...
      const res = await API.get(`/teachers?limit=${limit}&offset=${offset}&search=${searchText}`)
      if (res.data.status) {
  ...
  const handleStatusToggle = async (user) => {
    ...
    try {
      const response = await API.put(`/teachers/${user._id}`, {
                          ...
                          <td>
                            <div className="actionTable">
                              <NavLink
                                onClick={(e) => {
                                  e.preventDefault()
                                  navigate('/admin/edit-teacher/' + user._id)
                                }}
                              >
                                {/* <CIcon icon={cilPencil} custom="true" className="nav-icon" /> */}
                                <i className="fa fa-pencil"></i>
                              </NavLink>
                              <NavLink to={'/admin/view-teacher/' + user._id}>
