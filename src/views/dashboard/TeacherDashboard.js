...
//   cilCloudDownload,
// } from '@coreui/icons'
// import TeacherMainChart from './TeacherMainChart'
...
import TeacherMainChart from './TeacherMainChart'
import UserList from '../teacher/user-management/UserList'
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Fetch widgets + chart data
    const fetchAll = async () => {
      try {
        const [statsRes, chartRes] = await Promise.all([
          API.get(`teacher/dashboard/${teacher._id}`),
          API.get(`teacher/dashboard/${teacher._id}/student-counts?filter=${timeFilter.toLowerCase()}`),
        ])

        setStatistic(statsRes.data.data)
        ...
        <CCardBody>
          <CRow>
            <CCol sm={5}>
              ...
              <div className="small text-body-secondary">Time Filter: {timeFilter}</div>
            </CCol>
            <CCol sm={7} className="d-none d-md-block">
              {/* <CButton color="primary" className="float-end">
                <CIcon icon={cilCloudDownload} />
          ...
          {/* Chart Section */}
          <TeacherMainChart data={chartData} />
