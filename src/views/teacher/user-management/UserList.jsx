  ...
  const fetchUsers = async (pageNo = 1, searchText = '') => {
    ...
    try {
      ...
      const res = await API.get(
        `/teacher/users?limit=${limit}&offset=${offset}&search=${searchText}&teacher_id=${teacher._id}`
      )
      if (res.data.status) {
  ...
  const handleStatusToggle = async (user) => {
    ...
    try {
      ...
      if (response.status === 200) {
        Swal.fire('Success', 'Student status updated successfully!', 'success')
        setList((prev) =>
                          ...
                          <td>
                            <div className="actionTable">
                              <NavLink
                                onClick={(e) => {
                                  e.preventDefault()
                                  navigate('/teacher/users/' + user._id)
                                }}
                              >
                                <i className="fa fa-pencil"></i>
                              </NavLink>
                              <NavLink to={'/teacher/view-user/' + user._id}>
