...
router.post('/', async (req, res) => {
  try {
    var post = req.body;
    const user = await Quiz.create(post);
...
router.get('/', async (req, res) => {
  const users = await Quiz.find().sort({'created_at': -1});
  res.json(users);
});
...
router.put('/:id', async (req, res) => {
  try {
    const user = await Quiz.findByIdAndUpdate(
...
router.post('/purchase', auth, async (req, res) => {
  try {
    ...
    const plan = await Quiz.findById(post.plan_id);
    if (!plan) return res.status(404).json({ msg: 'Plan not found' });

    let expiry_date = new Date();
    if(Quiz.validity_type == 'year') {
      expiry_date.setFullYear(expiry_date.getFullYear() + parseInt(Quiz.validity_time));
    }
